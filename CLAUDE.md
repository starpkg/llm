...
- **Module construction.** `Module` wraps a `base.ConfigurableModule` (`cfgMod`) plus its `Extend()` accessor (`ext`), an optional injected `*oai.Client` (`cli`), and an optional host policy. `NewModule()` registers the seven config options with empty/preset defaults; `NewModuleWithConfig(provider, endpointURL, apiKey, gptModel, dalleModel, apiVersion)` presets them. Both retain the historical self-configuring behavior and route through `newModuleWithOptions`. `NewModuleWithHostPolicy(HostPolicy{...})` is the opt-in embedding path for untrusted scripts: provider, endpoint, and key are host-only; same-origin redirects are enforced; and an optional canonical file root contains `image_file` reads. `genConfigOption` is the shared builder that wires each option's name, description, default, and `LLM_<NAME>` env var; the API-key option is marked `SetSecret(true)`.
- **`LoadModule()`** registers three additional builtins — **`message`**, **`chat`**, **`draw`** — alongside the `set_<option>` / `get_<option>` builtins that `base.ConfigurableModule.LoadModule` generates from the config options. The API key is secret, so `set_openai_api_key` exists but `get_openai_api_key` does **not** (secrets are write-only from a script). In host-policy mode, the provider, endpoint, and API-key setters are omitted; invalid policy values are returned by the loader.
- **`message`** (`newMessageStruct`) builds a plain Starlark dict with the non-empty subset of `role`/`text`/`image`/`image_file`/`image_url`. It is data only — no API call. `chat`'s `messages=[...]` consumes exactly these dicts. In host-policy mode, `image_file` is resolved beneath the configured file root and symlink escapes are rejected.
- **`chat`** (`genChatFunc`) is the text path. `parseChatParams` unpacks 22 keyword args into `chatParams`; `prepareMessages` prepends an implicit user message built from the top-level `text`/`image*` args; `messagesToChatMessages` converts each dict to an `oai.ChatCompletionMessage` (text-only → `Content`; any image → `MultiContent` parts, with `image`/`image_file` base64-encoded into a `data:` URL via `imageDataToBase64`/`imageFileToBase64`); `prepareChatRequest` assembles the `oai.ChatCompletionRequest` (response_format text/json, reasoning_effort, kwargs → `ChatTemplateKwargs`). Then either `handleBlockingRequest` or `handleStreamingRequest`/`processStream` runs, both honoring `retry` and short-circuiting on HTTP 400. `formatChatResponse` returns a content string for `n==1`, a list for `n>1`, or the full converted response for `full_response=True`.
...
7. **Bounded requests, documented trust model.** `getClient` sets `cfg.HTTPClient = newHTTPClient()`, whose `Transport` bounds dial + TLS + response-header time at `request_timeout` (host/script-tunable, default 120s, non-positive → default, clamped so it can't overflow) so a slow/hanging endpoint can't stall the caller — the go-openai default config has no timeout, so don't drop this. It deliberately sets **no** total `http.Client.Timeout`: that is a hard cap on the whole body and would truncate a long streaming chat (`stream=True`) mid-stream. Blocking requests (chat without `stream`, draw) get a total deadline via `context.WithTimeout(ctx, requestTimeout)` so a headers-then-stall-body endpoint can't hang them; streaming gets no context deadline (so a long stream isn't cut off) and is bounded only through dial/TLS/response-header — a rare post-header pre-first-token stall on a stream is left to the caller's machine/thread timeout. Keep this split. The historical constructors are **self-config** (like `s3`): a script may set the provider/endpoint/key, so the consequences — a host-injected key can be sent to a script-chosen endpoint (SSRF/key-exfiltration), and `image_file` reads arbitrary host files (bounded to 64 MiB but not path-jailed) — are documented in the README + docs/API.md *Safety / trust model*. `NewModuleWithHostPolicy` fixes provider/endpoint/key, rejects cross-origin redirects, and optionally contains `image_file` below `FileRoot`; use it for untrusted scripts. The policy path is deliberately small and does not change the historical default.
...
- **GoDoc** — package comment + a doc comment on every exported symbol (`ModuleName`, the `Provider*` consts, `Module`, `HostPolicy`, `NewModule`, `NewModuleWithConfig`, `NewModuleWithHostPolicy`, `LoadModule`, `SetClient`), first word = symbol name (gated by `revive`'s `exported` rule in CI).
- **doc-coverage gate** — `go run github.com/1set/meta/doccov@master .` must exit 0. doccov statically scans the package for `starlark.NewBuiltin(...)` calls, resolves each builtin's name (including the `ModuleName + ".<name>"` form `chat`/`draw` use, where the trailing segment is the script-facing name), and checks each shows up as a backtick word in the README. The `set_*`/`get_*` builtins are generated in `base` with computed names, so doccov does not see them — document them anyway for accuracy (it is the right thing, just not gate-enforced here).
